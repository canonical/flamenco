...
public class DpkgChangelogReaderTests
{
    ...
    public async Task Test()
    {
        ...
        using var changelogReader = DpkgChangelogReader.FromString(changelog);

        var entry = await changelogReader.ReadChangelogEntryAsync().ConfigureAwait(false);
        Assert.Null(await changelogReader.ReadChangelogEntryAsync().ConfigureAwait(false)); 
        
        Assert.NotNull(entry);
        
        Assert.Equal(expected: "dotnet7", actual: entry.PackageName);
        Assert.Equal(expected: "7.0.118-0ubuntu1~24.04.1~ppa1", actual: entry.Version.ToString());
        Assert.Equal(expected: new [] {"noble"}, actual: entry.Distributions);
